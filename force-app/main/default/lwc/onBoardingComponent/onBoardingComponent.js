...
export default class OnBoardingComponent extends NavigationMixin(
  LightningElement
) {
  @api recordId;
  @track checklistList = [];
  ...
  getallatachment() {
    getAllAttachmenterlatedtoTask({ recordId: this.recordId })
      .then((result) => {
        console.log(
          "taskid and attachment exist::1::" + JSON.stringify(result)
        );
        if (result != "") {
          console.log(
            "taskid and attachment exist::2::" + JSON.stringify(result)
          );
  ...
  handlepreviewdoc(event) {
    console.log(
      "event.currentTarget.taskid:::::" + event.currentTarget.contentdocid
    );
    console.log(
      "event.currentTarget.taskid:::::" +
        event.currentTarget.dataset.contentdocid
    );
          ...
          if (item.Tasks) {
            ...
            item.Tasks.filter((el) => {
              ...
              this.attachmentList.filter((attchitem) => {
                console.log(
                  "taskid:LinkedEntityId:otuside::" +
                    el.Id +
                    ": taskid::" +
                    attchitem.LinkedEntityId
                );
  ...
  handleNewChecklistSave() {
    this.load = true;
    createNewCaseChecklist({
      caseRecId: this.recordId,
      checkList: this.newChecklist.Name
    })
      .then((result) => {
  ...
  handleSendApproval() {
    ...
    updateCase({
      ...
      submitterComment: this.caseApprovalComment
    })
      .then((result) => {
        if (result == "error") {
          this.showToastMessage("Error", "Something went wrong.", "error");
  ...
  handlePickListValue(event) {
    ...
    switch (fieldName) {
      case "link":
        this.currentChecklist.Tasks = this.currentChecklist.Tasks.filter(
          (el) => {
            if (el.Id == ProductId) {
              el.Link__c = value;
  ...
  handleSaveAddTask() {
    ...
    this.load = true;
    createTask({
      taskduedate: this.newTaskDuedate,
      taskSubject: this.newTaskSubject,
      whatId: this.currentChecklist.Id
    })
      .then((result) => {
        if (result == "success") {
