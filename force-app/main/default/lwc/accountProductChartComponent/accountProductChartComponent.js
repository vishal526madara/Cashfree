...
export default class AccountProductChartComponent extends LightningElement {
  ...
  circleClass =
    "icon-circle icon-circle-blue slds-m-around_xx-small slds-align_absolute-center";
  noCircleClass =
    "icon-circle slds-m-around_xx-small slds-align_absolute-center";

  blueSquare =
    "slds-grid slds-wrap icon-square icon-circle-blue slds-align--absolute-center";
  greySquare =
    "slds-grid slds-wrap icon-square icon-circle-grey slds-align--absolute-center";
        ...
        if (result.length == 0) {
          ...
          this.oppPro = result.filter((item) => {
            if (item.checked) {
              ...
              item.class =
                "slds-icon_container slds-icon-custom-custom62 icon-style slds-align_absolute-center active-effect active-background";
              // item.circleClass=this.circleClass;
            } else {
              item.effect = "inactive-effect";
              item.class =
                "slds-icon_container slds-icon-custom-custom62 icon-style slds-align_absolute-center inactive-effect inactive-background";
