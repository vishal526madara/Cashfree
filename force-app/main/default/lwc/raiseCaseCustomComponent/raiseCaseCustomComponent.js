...
export default class RaiseCaseCustomComponent extends LightningElement {
    ...
    connectedCallback() {
        console.log('Case For Data Type::::'+ typeof this.caseForName);
        if(!this.caseForName){
            this.caseForValue = this.caseForName;
            console.log('line 32 caseForValue: '+this.caseForValue);            
        }
    ...
    handleTickButton(event) {
        ...
        console.log('Discrepency List::::',+ JSON.stringify(this.discrepancyList));
    }
    ...
    @track insertionOutput = false;
      insertCasesOnSave() {
        ...
        if(this.caseForValue == undefined){
            this.showToastMessage('Error','Please Select Case For...','error');
        ...
        let inputList=[];
        for(let i=0; i<this.discrepancyList.length; i++){
            this.discrepancyList[i].caseForValue = this.caseForValue;
            if(this.discrepancyList[i].checkedValue == true){
                  inputList.push(this.discrepancyList[i]);
            }
            console.log('Data::::'+ this.discrepancyList[i].checkedValue);
        }
        ...
        console.log('Insert Calledd:::');
         insertCases({ recordId: this.recordId, casevalueList: inputList })
            .then(result => {
        ...
        //await this.validationRecord(this.discrepancyList);
    ...
    */
    ...
    validationRecord(discrepancyList) {
        
        console.log('Function Called:::'+ discrepancyList.length());
        if (discrepancyList.length == 0) {
            this.showToastMessage('Error', 'Please Select a Discrepancy.', 'error');
            return true;
        }
        if(this.caseForName != undefined){
            this.showToastMessage('Error','Please Select Case For...','error');
            return;
        }
        return false;
