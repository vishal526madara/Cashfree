...
export default class ProductComponent extends LightningElement {
    ...
    handleAccordian(event){
        ...
        switch(name){
            case 'modes':
                this.accordianClass.mode=(value===this.closeAccordian)?this.openAccordian:this.closeAccordian;
                break;
            case 'settle':
                this.accordianClass.settle=(value===this.closeAccordian)?this.openAccordian:this.closeAccordian;
                break;
        }
    }
    ...
    handleNavigation(event){
        ...
        if(checked){
            this.modesOptions.settlementList=this.modesOptions.settlementList.filter(item=>{
                item.check=false;
                return item;
            });
    ...
    handleSettlement(event){
        var price=event.currentTarget.dataset.id==null?0:event.currentTarget.dataset.id;
        var checked=event.target.checked;
        var name=event.target.name;
        var value=event.target.value;
        ...
        if(checked){
            ...
            this.modesOptions.settlementList.filter(item=>{
                if(item.Id==value){
                    this.settlementList.push({
                        ...
                        Commercial_Price__c:item.Commercial_Price__c==null?0:item.Commercial_Price__c
                    });
                }
            });
            var addOnPrice=0;
            ...
            this.commercialList.filter(item=>{
                ...
                item.actualPricePercent=Number(item.Commercials__c)+Number(item.Add_On__c);
                item.actualPrice=Number(item.Commercial_Price__c)+Number(item.Add_On_Price__c);
            });
            this.saveList.filter(item=>{
                ...
                item.actualPricePercent=Number(item.Commercials__c)+Number(item.Add_On__c);
                item.actualPrice=Number(item.Commercial_Price__c)+Number(item.Add_On_Price__c);
                item.Actual_Price_Percent__c=Number(item.Commercials__c)+Number(item.Add_On__c);
                item.Actual_Price__c=Number(item.Commercial_Price__c)+Number(item.Add_On_Price__c);
            });
        }else{
            ...
            this.commercialList.filter(item=>{
                item.actualPricePercent=item.actualPricePercent-this.settlementTotal.Commercial__c;
                item.actualPrice=item.actualPrice-this.settlementTotal.Commercial_Price__c;
                item.Add_On__c=addOn;
                item.Add_On_Price__c=addOnPrice;
            });
            this.saveList.filter(item=>{
                item.actualPricePercent=item.actualPricePercent-this.settlementTotal.Commercial__c;
                item.Actual_Price_Percent__c=item.Actual_Price_Percent__c-this.settlementTotal.Commercial__c;
                item.actualPrice=item.actualPrice-this.settlementTotal.Commercial_Price__c;
                item.Actual_Price__c=item.Actual_Price__c-this.settlementTotal.Commercial_Price__c;
                item.Add_On__c=addOn;
                item.Add_On_Price__c=addOnPrice;
            });
            ...
            case 'addOnPrice':
                this.commercialList.filter(item=>{
                    if(item.Id==productId){
                        ...
                        item.actualPrice=item.Commercial_Price__c+Number(item.Add_On_Price__c);
                    }
                });
                this.saveList.filter(item=>{
                    if(item.Id==productId){
                        item.Add_On_Price__c=value;
                        item.actualPrice=item.Commercial_Price__c+Number(item.Add_On_Price__c);
                        item.Actual_Price__c=item.Commercial_Price__c+Number(item.Add_On_Price__c);
                    }
                });
                break;
        }
    ...
    handleSave(){
        ...
        saveProduct({productRec:this.selectedProduct,oppProList:this.saveList,settlementList:this.settlementList})
        .then(result=>{
