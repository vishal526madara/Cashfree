...
export default class LeadScoringComponent extends NavigationMixin(
  LightningElement
) {
  @api recordId;
  ...
  getLead() {
    ...
    getLeadDetails({ recordId: this.recordId })
      .then((result) => {
        ...
        console.log(
          "this.leadList from apex::  " + JSON.stringify(this.leadList)
        );
        if (this.leadList != null) {
          for (let i = 0; i < this.leadList.childWrapperList.length; i++) {
            if (this.leadList.childWrapperList[i].field == "Website") {
              this.websitePercentage =
                this.leadList.childWrapperList[i].weightage;
              this.websitePoints = this.leadList.childWrapperList[i].points;
              //}else if(this.leadList.childWrapperList[i].field =='LeadSource'){
            } else if (
              this.leadList.childWrapperList[i].field == "Lead_Type__c"
            ) {
              this.leadSourcePercentage =
                this.leadList.childWrapperList[i].weightage;
              this.leadSourcePoints = this.leadList.childWrapperList[i].points;
            } else if (
              this.leadList.childWrapperList[i].field == "Action_Type__c"
            ) {
              this.actionTypePercentage =
                this.leadList.childWrapperList[i].weightage;
              this.actionTypePoints = this.leadList.childWrapperList[i].points;
            } else if (
              this.leadList.childWrapperList[i].field == "Campaign_Type__c"
            ) {
              this.campaignTypePercentage =
                this.leadList.childWrapperList[i].weightage;
              this.campaignTypePoints =
                this.leadList.childWrapperList[i].points;
            } else if (
              this.leadList.childWrapperList[i].field == "Campaign_Sub_type__c"
            ) {
              this.campaignSubTypePercentage =
                this.leadList.childWrapperList[i].weightage;
              this.campaignSubTypePoints =
                this.leadList.childWrapperList[i].points;
            }
          }
        ...
        console.log(
          "this.leadList after conversion  " + JSON.stringify(this.leadList)
        );
        console.log(
          " this.leadList child's length  " +
            this.leadList.childWrapperList.length
        );
