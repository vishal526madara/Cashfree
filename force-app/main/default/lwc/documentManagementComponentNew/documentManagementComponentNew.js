...
export default class DocumentManagementComponentNew extends NavigationMixin(
  LightningElement
) {
  @api componentHide;
  ...
  getAllDocumentSync() {
    getAllSyncedDocs({ recordId: this.recordId })
      ...
      .catch((error) => {
        ...
        this.toast(
          "Error",
          "Unable to Fetch Document from CF System",
          "error",
          "pester"
        );
      });
            ...
            if (result[i].Node__c == "Regular") {
              ...
              if (this.regularListMap.length > 0) {
                ...
                if (flag == 0) {
                  ...
                  this.regularListMap.push({
                    key: result[i].Document_Type__c,
                    value: this.tempList
                  });
                }
                console.log(
                  "regularListMap: " + JSON.stringify(this.regularListMap)
                );
              } else {
                console.log(
                  "regularListMap: " + JSON.stringify(this.regularListMap)
  ...
  checkIfOppOrNotFunction() {
    checkIfOppClosedOrNot({ recordId: this.recordId })
      .then((result) => {
        ...
        if (result == "Yes") {
          this.toast(
            ...
            "Atleast upload one document to close the opportunity",
            "error",
            "sticky"
          );
        }
      ...
      .catch((error) => {
        console.log(
          "checkIfOppOrNotFunction (Error) ::: " + JSON.stringify(error)
        );
        this.toast(
          "Error",
          "Something is wrong, please contact your system administrator",
  ...
  getNodeTypeFunction() {
    ...
    getNodeType()
      .then((result) => {
        ...
        console.log(
          "Get Node Type (kycTypeList)  :: " + JSON.stringify(this.kycTypeList)
        );
        this.spinnerOn = false;
      })
      .catch((error) => {
  ...
  handleHideKycCategory(event) {
    console.log(
      "Name Value (handleHideKycCategory) " + event.target.dataset.kycvalue
    );
    for (let i = 0; i < this.kycTypeList.length; i++) {
      if (
        this.kycTypeList[i].kycCategoryValue == event.target.dataset.kycvalue
      ) {
  ...
  handleShowKycCategory(event) {
    console.log(
      "Name Value (handleShowKycCategory) " + event.target.dataset.kycvalue
    );
    for (let i = 0; i < this.kycTypeList.length; i++) {
      if (
        this.kycTypeList[i].kycCategoryValue == event.target.dataset.kycvalue
