...
export default class DocumentManagementComponentNewLevelFour extends NavigationMixin(
  LightningElement
) {
  @api recordId;
  @api documentTypeId;
  ...
  userDetails({ error, data }) {
    if (error) {
      ...
      if (
        this.userProfileName == "Operations - Merchant Onboarding" ||
  ...
  getFieldPicklistValueFunction() {
    ...
    getFieldPicklistValue()
      ...
      .catch((error) => {
        ...
        this.toast(
          "Error",
          "Something is wrong During getting Picklist from DB, please contact your system administrator",
          "error",
          "pester"
        );
      });
  ...
  getDocumentListFunction() {
    ...
    getDocumentList({
      ...
      .then((result) => {
        ...
        console.log(
          "Document List >>> :: " + JSON.stringify(this.documentList)
        );
        for (let i = 0; i < this.documentList.length; i++) {
          ...
          if (this.documentList[i].decisionAction == "Approved") {
            ...
            if (this.loggedInIsAM) this.documentList[i].allowUpload = false;
            else this.documentList[i].allowUpload = true;
          }
        }
  ...
  apiCallout() {
    insert_OR_Method_Callout({
      ...
      .then((result) => {
        console.log(
          "result.MVDResponseWrapperObj >> ",
          result.MVDResponseWrapperObj
        );
        console.log("Result Print :: ", result);
        console.log(
          "presignedURL >> ",
          result.MVDResponseWrapperObj.data.presignedURL
        );
        if (result) {
      ...
      .catch((error) => {
        ...
        this.toast(
          "Error",
          "Something is wrong During API Callout, please contact your system administrator",
          "error",
          "pester"
        );
      });
  ...
  handleDeleteFile(event) {
    this.spinnerOn = true;
    console.log(
      "Content Document Id Print :: " + event.currentTarget.dataset.docid
    );
    console.log("Recor Id Print :: " + event.currentTarget.dataset.recid);
    this.deleteFileFunction(
      event.currentTarget.dataset.docid,
      event.currentTarget.dataset.recid
    );
  }
  ...
  handleUploadFinished(event) {
    ...
    console.log(
      "uploadedFiles[0].contentVersionId  ::  ",
      uploadedFiles[0].contentVersionId
    );
    ...
    if (
      this.fileSizeUpload == "" ||
      this.fileSizeUpload == undefined ||
      this.fileSizeUpload == null
    ) {
      this.controlUploadEngine(
        nameVarTemp,
        documentIdVarTemp,
        uploadedNameVarTemp,
        contentVersionIdVarTemp,
        recidVarTemp,
        documenttextVarTemp,
        accidVarTemp
      );
      if (
        event.target.dataset.recid == "" ||
        event.target.dataset.recid == undefined
      ) {
        let folderObject = {
    ...
    getFileSize({ docId: docIdVar, fileSizeInMb: fileSizeInMbVar })
      .then((result) => {
        if (result == "Invalid Size") {
          this.toast(
            "Error",
            "Invalid size, file size cannot exceed " + fileSizeInMbVar + " MB",
            "error",
            "pester"
          );
          this.spinnerOn = false;
      ...
      .catch((error) => {
        console.log(
          "error (createDocumentFunction) ::" + JSON.stringify(error)
        );
  ...
  handleuploadfile(event) {
    ...
    console.log(
      "<<<  : : :  : : :  >>>  handleuploadfile : : :  >>>  contentVersionId ",
      contentVersionId
    );

    const contentBodyId = uploadedFiles[0].contentBodyId;
    console.log(
      "<<<  : : :  : : :  >>>  handleuploadfile : : :  >>>  contentBodyId ",
      contentBodyId
    );
  ...
  uploadFile(filetype, contentVersionId, fileName) {
    ...
    console.log(
      "inside this.merchantidupload new function" + this.merchantidupload
    );
    ...
    uploadmerchant_document({
      merchantId: this.merchantidupload,
  ...
  getUploadedFileSizeChecker(filetype, contentVersionId, fileName) {
    console.log(
      "f1 " + filetype + " f2 " + contentVersionId + " f3 " + fileName
    );
  ...
  createDocumentFunction(documentLinkObject, folderObjectVar, presentVar) {
    this.spinnerOn = true;
    createDocument({
      contentDocumentLinkObject: documentLinkObject,
      folderObject: folderObjectVar,
      present: presentVar
    })
      .then((result) => {
        if (result == "Done") {
  ...
  handleOpenUpload(event) {
    ...
    console.log("recid :: ??????? " + event.currentTarget.dataset.recid);
    console.log(
      "documenttext :: ??????? " + event.currentTarget.dataset.documenttext
    );
