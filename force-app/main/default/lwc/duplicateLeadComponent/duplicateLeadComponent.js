...
export default class DuplicateLeadComponent  extends NavigationMixin(LightningElement) {
    @api recordId;
    // @api objectApiName;
    ...
    connectedCallback(){
        ...
        console.log(' :: Duplicate Connected Callback :: (Record Id) :: '+this.recordId);
        this.getLead();
    }

    getLead(){
        console.log(' Duplicate Event record id While querying Lead record List>>>    '+this.recordId);
        getLeadRecords({recordId:this.recordId})
        .then(result => {
            console.log("result::",result);
            this.leadRecordList = result;
            console.log("🚀 >>>> Duplicate this.leadRecordList", this.leadRecordList);
            console.log("🚀 >>>>>Duplicate  this.leadRecordList", this.leadRecordList.length);
