...
getDcocument(){ 
this.load = true;
documentidList({recordId  : this.recordId}).then(result=>{ 
    console.log('11 getContentDocumentList :: >>>  '+JSON.stringify(result)); 
    this.documentId = result;
    console.log('11 getContentDocumentList :: >>>  '+JSON.stringify(this.documentId)); 
    ...
    console.log('11 after uniqueId :: >>>  '+JSON.stringify(this.documentId)); 
    this.doComments = true;
    // this.dispatchEventMethod('File uploaded successfully','success');   
    this.load = false; 
         
}).catch(error=>{ 
    console.log('Error (getContentDocumentList) ::: '+JSON.stringify(error));
...
redirectToContentDocument(event) {
    ...
    window.location.href = '/lightning/r/ContentDocument/'+contentId+'/view';
}
