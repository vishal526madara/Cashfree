...
export default class DocumentManagementComponentNewLevelThree extends LightningElement {
    ...
    getDocumentTypeFunction(){
        this.spinnerOn = true;
        getDocumentType({categoryId : this.categoryId, categoryValue : this.categoryValue, 
            kycCategoryValue: this.kycCategoryValue, recordId : this.recordId})
        .then(result=>{
            console.log('Category Type :: '+result);
            this.documentTypeList = JSON.parse(result);
            console.log('documentTypeList  (Size) :: '+this.documentTypeList.length);
            console.log('documentTypeList  :: '+JSON.stringify(this.documentTypeList));
    ...
    handleHideDocument(event){
        console.log('Name Value (handleHideDocument) '+event.target.dataset.docvalue);
        console.log('Name Value (handleHideDocument) id '+event.target.dataset.docid);
        for(let i =0; i<this.documentTypeList.length; i++){
            if(this.documentTypeList[i].documentTypeId == event.target.dataset.docid){
    ...
    handleShowDocument(event){
        console.log('Name Value (handleShowDocument) '+event.target.dataset.docvalue);
        console.log('Name Value (handleShowDocument) id'+event.target.dataset.docid);
        for(let i =0; i<this.documentTypeList.length; i++){
            if(this.documentTypeList[i].documentTypeId == event.target.dataset.docid){
