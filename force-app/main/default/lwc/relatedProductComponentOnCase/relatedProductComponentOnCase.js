...
export default class RelatedProductComponentOnCase extends LightningElement {
    ...
    async getOpportunityData(){
        ...
        .then(result=>{
            ...
            if(result.OppPro!=null){
                ...
                this.oppProduct.OppPro.ProductImage=this.oppProduct.OppPro.Product__r.Product_Image__c;
                this.recordList=this.oppProduct.oppModeList.filter(item=>{
                    //item.ModeName=item.Mode__r.Name;
                    item.nameUrl='/'+item.Id;
                    item.CommPercent=item.Commercial__r.Commercials__c==null?0:item.Commercial__r.Commercials__c;
                    item.CommPrice=item.Commercial__r.Commercial_Price__c==null?0:item.Commercial__r.Commercial_Price__c;
                    return item;
                });
                this.oppProduct.settlementList.filter(item=>{
                    ...
                    item.Commercial__c=item.Commercial__c==null?0:item.Commercial__c;
                });
    ...
    handleReset(){
        ...
        .then(result=>{
            if(result=='error'){
                ...
                this.showToastMessage('Success','Product Deleted successfully.','success');
                this.showConfirmPopOver=false;
                this.handleCancel();
            }
        })
    ...
    handleInitiateBankOpsOpen(){
        ...
        .then(result => {
            if(result == 'Fail'){
             this.showToastMessage('Warning......','Not able to Initiate the Bank Ops,Please Check the MID', 'warning');
             return;
            }
